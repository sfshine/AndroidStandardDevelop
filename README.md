...
考虑到 Android 众多的 UI 控件，为避免控件和普通成员变量混淆以及更好地表达意思，可使用如下两种方式命名
1.(推荐)直接使用xml中控件的id的名字作为变量名（具体见附录 [UI 控件缩写表](#ui-控件缩写表)）。
例如 'btn_avatar'、 'fl_container'。
 
2.所有用来表示控件的成员变量统一加上控件缩写作为前缀（具体见附录 [UI 控件缩写表](#ui-控件缩写表)）。
